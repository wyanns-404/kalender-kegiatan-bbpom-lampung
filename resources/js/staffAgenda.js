...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        const res = await fetch("/agenda/staff");
    ...
    details.innerHTML = `
        <div class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                ${
                    event.extendedProps.private_content
                        ? `
                <div class="flex items-start space-x-4 p-4 bg-gray-50 rounded-xl">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-bold text-gray-900 mb-1">Private Content</p>
                        <p class="text-gray-700 leading-relaxed">${event.extendedProps.private_content}</p>
                    </div>
                </div>
                `
                        : ""
...
async function showEventFromSidebar(eventUuid) {
    ...
    if (event) {
        ...
        if (event.private_content) extendedProps.private_content = event.private_content;
        const fcEvent = {
            title: event.title,
            start: event.start,
            end: event.end,
